...
class _DateTimeScreenState extends State<DateTimeScreen> {
  ...
  List<Map<String, dynamic>> repairRequsets = [];
  List<Map<String, dynamic>> paintingRequests = [];
  Timer? _timer;

  bool _isRepairSortedAscending = true;
  bool _isPaintingSortedAscending = true;
  @override
  void initState() {
    super.initState();
    _startTimer();
  ...
  Future<void> _deleteRequest(int id) async {
    try {
      ...
      await db.delete('repair_requests', where: 'id = ?', whereArgs: [id]);
      await _fetchData();
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _fetchData() async {
    try {
      final repairData = await DatabaseHelper().getRepairRequest();
      final paintingData = await DatabaseHelper().getPaintingRequests();
      if (mounted) {
        setState(() {
          repairRequsets = repairData;
  ...
  void _sortRepairRequests(String criteria) {
    setState(() {
      repairRequsets = List.from(repairRequsets)
        ..sort((a, b) {
          int result = 0;
          if (criteria == 'name') {
  ...
  Widget _buildRepairRequestsTable() {
    return repairRequsets.isEmpty
        ? const Center(
            child: Text('Нет данных для отображения заявок на ремонт'))
        : SingleChildScrollView(
            ...
            child: DataTable(
              ...
              rows: repairRequsets.map((request) {
  ...
  Widget _buildPaintingRequestsTable() {
    return paintingRequests.isEmpty
        ? const Center(
            child: Text('Нет данных для отображения заявок на покраску'))
        : SingleChildScrollView(
            scrollDirection: Axis.horizontal,
            child: DataTable(
              columns: const [
  ...
  Future<void> _markPaintingAsCompleted(int id) async {
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Статус заявки на поркаску обновлен на "выполнено')),
      );
    } catch (e) {
