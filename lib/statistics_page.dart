...
class StatisticsPage extends StatelessWidget {
  ...
  Future<Map<String, dynamic>> _fetchStatistics() async {
    final repairRequests = await DatabaseHelper().getRepairRequest();
    ...
    int inProgressRepairs = repairRequests
        .where((request) => request['status'] == 'в работе')
        .length;
    int completedRepairs = repairRequests
        .where((request) => request['status'] == 'выполнено')
        .length;

    int inProgressPaintings = paintingRequests
        .where((request) => request['status'] == 'в работе')
        .length;
    int completedPaintings = paintingRequests
        .where((request) => request['status'] == 'выполнено')
        .length;
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return const Center(child: Text('Ошибка загрузки статистики'));
              ...
              (snapshot.data!['repairRequests'].isEmpty &&
                  snapshot.data!['paintingRequests'].isEmpty)) {
            return const Center(child: Text('Нет данных для отображения'));
          } else {
            final stats = snapshot.data!;
            final repairRequests =
                stats['repairRequests'] as List<Map<String, dynamic>>;
            final paintingRequests =
                stats['paintingRequests'] as List<Map<String, dynamic>>;
